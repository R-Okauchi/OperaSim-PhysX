...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    sqrdists = torch.sum(
        (xyz.unsqueeze(1) - new_xyz.unsqueeze(2)) ** 2, -1
    )  # (B, S, N)
    group_idx[sqrdists > radius**2] = N
    group_idx = group_idx.sort(dim=-1)[0][:, :, :nsample]
    group_idx[group_idx == N] = (
        group_idx[:, :, 0].unsqueeze(-1).repeat(1, 1, nsample)[group_idx == N]
    )
    return group_idx
...
class FeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            ...
            dists = torch.sum(
                (xyz1.unsqueeze(2) - xyz2.unsqueeze(1)) ** 2, dim=-1
            )  # (B, N, S)
...
class PointNet2Seg(nn.Module):
    def __init__(self, num_classes=6):
        ...
        self.sa1 = SetAbstraction(
            npoint=512, radius=0.2, nsample=32, in_channels=3, mlp=[32, 64, 128]
        )
        self.sa2 = SetAbstraction(
            npoint=128, radius=0.4, nsample=64, in_channels=128 + 3, mlp=[128, 128, 256]
        )
        self.sa3 = SetAbstraction(
            npoint=32, radius=0.8, nsample=64, in_channels=256 + 3, mlp=[256, 512, 1024]
        )
    ...
    def forward(self, xyz, points=None):
        ...
        l0_points = self.fp1(
            xyz,
            l1_xyz,
            points.permute(0, 2, 1) if points is not None else None,
            l1_points,
        )

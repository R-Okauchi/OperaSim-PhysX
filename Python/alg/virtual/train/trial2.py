...
class SegmentationDataset(Dataset):
    def __init__(
        self, root="dataset", num_points=200000, voxel_size=0.05, transform=None
    ):
        self.root = root
    ...
    def voxelize_point_cloud(self, pcd, labels):
        ...
        # 並列処理でラベルを割り当て
        with mp.Pool(processes=mp.cpu_count()) as pool:
            voxel_labels = pool.map(
                partial(voxelize_point_cloud_worker, tree=tree, labels=labels),
                voxel_points,
            )
...
def main():
    ...
    try:
        ...
        dataset = SegmentationDataset(
            root=root_dir, num_points=num_points, voxel_size=voxel_size
        )
        ...
        for epoch in range(10):
            ...
            for i in range(len(train_loader)):
                ...
                try:
                    ...
                    print(
                        f'Current Allocated Memory: {torch.cuda.memory_allocated(device)/1e9 if device.type == "cuda" else torch.mps.current_allocated_memory()/1e9:.2f} GB'
                    )
                    print(
                        f'Current Cached Memory: {torch.cuda.memory_reserved(device)/1e9 if device.type == "cuda" else torch.mps.driver_allocated_memory()/1e9:.2f} GB'
                    )
        ...
        with open("training_data.csv", mode="w", newline="") as file:
            ...
            for epoch in range(10):
                writer.writerow(
                    [epoch + 1, train_losses[epoch], val_losses[epoch], lrs[epoch]]
                )

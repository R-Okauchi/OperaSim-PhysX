...
output_dir = "/Users/ryout/Documents/main_desk/omniverse_driver/field_output3"
usd_files = [
...
print(dir(context))
print(help(context.open_stage_async))
...
def load_and_place_usd_files(stage, usd_files, field_size):
    """
    Load the specified USD files into the current stage and place them randomly on the field.
    """
    placed_prims = []  # Track the placed Prim paths
    for usd_file in usd_files:
        # Create a reference to the USD file in the current stage
        prim_path = Sdf.Path(
            f"/{os.path.basename(usd_file).split('.')[0]}"
        )  # Use the file name as the Prim name
        root_prim = stage.DefinePrim(prim_path)
        if not root_prim:
            print(f"Warning: Could not define Prim for USD file {usd_file}. Skipping.")
            continue

        # Set the reference to the USD file
        root_prim.GetReferences().AddReference(usd_file)

        # Place the object randomly on the field
        place_objects_on_field(stage, field_size, root_prim)
        placed_prims.append(root_prim)

    return placed_prims


# USDファイルをロードしてランダムに配置
placed_prims = load_and_place_usd_files(stage, usd_files, FIELD_SIZE)

# カメラとレンダリングの部分は既存のまま継続
...
with rep.new_layer():
    ...
    with rep.trigger.on_frame(num_frames=3, rt_subframes=64):
        pass
